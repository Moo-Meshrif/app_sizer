...
/// Core PreScale generator logic.
///
/// This file lives in `tool/` intentionally:
///   - `tool/` is never exported through `lib/`, so `dart:io` cannot
///     bleed into the public API surface.
///   - pub.dev's Web-compatibility check only traverses imports reachable
///     from `lib/`. Files in `tool/` and `bin/` are excluded from that check.
///
/// Entry point: [runGenerator].
/// Called by `bin/generate_prescale.dart`.

/// The entry point for the PreScale generator.
///
/// Scans the project for responsive extensions and generates a cache warmup function.
void runGenerator(List<String> args) {
  Logger.info('🚀 Starting PreScale Generator...');
...
void _runMultiTargetGeneration() {
  Logger.info('🧐 Running Smart Multi-Target Mode...');
  ...
  Logger.success('✨ All targets generated successfully!');
}
...
void _generateFor(
    ...
    {bool silent = false}) {
  if (!silent) {
    Logger.info('📂 Scanning ${scanPaths.join(', ')} -> $outputPath');
  }
  ...
  final dgNumbers = <double>{};
  final dmNumbers = <double>{};

  final regex = RegExp(r'(\d+(?:\.\d+)?)\.(w|h|vGap|hGap|sp|r|dg|dm)');
  ...
  writeList(dgNumbers, 'dg');
  writeList(dmNumbers, 'dm');
  buffer.writeln('}');
...
void _injectIntoFile(File file, String outputPath, String functionName,
  ...
  final importLine =
      "import '${_getRelativePath(file.path, outputPath)}';";
...
/// Simple ANSI logger for CLI output.
class Logger {
